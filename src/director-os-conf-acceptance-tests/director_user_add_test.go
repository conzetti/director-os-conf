...
var _ = Describe("UserAdd", func() {
	It("allows users to add users to the VM", func() {
		By("adding the users to /etc/passwd", func() {
			session := boshSSH("director-os-conf/0", "sudo cat /etc/passwd")
		...
		By("adding a password for the password user", func() {
			session := boshSSH("director-os-conf/0", "sudo cat /etc/shadow")
		...
		By("adding an authorized key for the key user", func() {
			session := boshSSH("director-os-conf/0", "sudo cat /home/test-user-key/.ssh/authorized_keys")
			Eventually(session, 30*time.Second).Should(gbytes.Say("test-user-public-key"))
			Eventually(session, 30*time.Second).Should(gexec.Exit(0))

			session = boshSSH("director-os-conf/0", "sudo ls -larth /home/test-user-password/.ssh")
		...
		By("adding an authorized key for the key-and-password user", func() {
			session := boshSSH("director-os-conf/0", "sudo cat /home/test-user-key-and-password/.ssh/authorized_keys")
			Eventually(session, 30*time.Second).Should(gbytes.Say("test-user-public-key"))
			Eventually(session, 30*time.Second).Should(gexec.Exit(0))
		})

		By("adding them to the bosh_sshers group", func() {
			session := boshSSH("director-os-conf/0", "sudo grep bosh_sshers /etc/group")
		...
		By("adding the apropriate user to the bosh_sudoers group", func() {
			session := boshSSH("director-os-conf/0", "sudo grep bosh_sudoers /etc/group")
